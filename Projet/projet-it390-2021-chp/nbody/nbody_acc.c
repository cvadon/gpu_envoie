...
void MoveParticles(const int nParticles, struct ParticleType* const particle, const float dt) {
  ...
  #pragma acc parallel loop present(particle)
...
{
  const double tStartglob = omp_get_wtime();
  FILE* fichier = NULL;

  fichier = fopen("test_parti_acc.txt", "w");
  ...
  for (int step = 1; step <= nSteps; step++) {
    ...
    if (fichier != NULL)
    {
      fprintf(fichier," %d  %f  %f  %f \n",step, particle[0].x, particle[0].y, particle[0].z); // Écriture du caractère A
  ...
  free(particle);
  const double tEndglob = omp_get_wtime();
  fprintf(fichier,"Temps global : %f  \n",tEndglob-tStartglob);
